...
function AddReview2(props) {
  const { user, SelectedMovie, NewReviewUpdated } = useSelector((state) => state);
  ...
  const [showModal, hideModal] = useModal(() => (
    <ReactModal isOpen>
      ...
      <RegisterLoginContainer dispatch={dispatch} hideModal={hideModal} />
    </ReactModal>
  ));
  ...
  const handleNewReviewRating = (e) => {
    ...
    if (validateForms()) {
      ...
      Axios.put(`http://localhost:5000/movies/update/${SelectedMovie.id}`, reviewUpdate)
        //   api
        //     .posts()
      ...
      {SelectedMovie && (
        <form onSubmit={handleNewReviewRating}>
          ...
          <div className="AddRating_rating_star">
            ...
            <StarRating rating={rating} setRating={setRating} id={SelectedMovie.id} />
